...
class StudentScoreManagerApp:
    ...
    def export_score_to_csv(self):
        if not self.score_data:
            messagebox.showwarning("Cảnh báo", "Không có dữ liệu điểm để xuất.")
            return

        # 1. Hỏi người dùng vị trí lưu file
        file_path = filedialog.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("CSV files", "*.csv"), ("All files", "*.*")],
            title="Lưu Bảng Điểm thành File CSV"
        )
        ...
        try:
            # 2. Định nghĩa tiêu đề (Headers) và chuẩn bị dữ liệu
            headers = ["Mã SV", "Tên Sinh Viên", "Môn học", "Học kỳ", "Tx1", "Tx2", "CK", "Điểm TB"]

            data_to_write = []
            for score in self.score_data:
                ma_sv = score['ma_sv']
                # Lấy tên sinh viên
                ten_sv = self.student_data.get(ma_sv, {}).get('ten', 'Không rõ')
            ...
            # 3. Ghi ra file CSV (sử dụng 'utf-8-sig' để hiển thị tiếng Việt chính xác trong Excel)
            with open(file_path, mode='w', newline='', encoding='utf-8-sig') as file:
                writer = csv.writer(file)
                writer.writerow(headers)
                writer.writerows(data_to_write)

            messagebox.showinfo("Thành công", f"Đã xuất dữ liệu điểm thành công ra:\n{os.path.basename(file_path)}")

        except Exception as e:
            messagebox.showerror("Lỗi Xuất File", f"Đã xảy ra lỗi khi ghi file: {e}")
    ...
    def create_tab_quan_ly_diem(self, notebook):
        ...
        # Nút Xuất File Excel (CSV) đã được thêm vào
        ttk.Button(button_frame, text="Xuất File Excel (CSV)", command=self.export_score_to_csv).pack(side="left",
                                                                                                      padx=20)
